...
export default function CoursesByProgram({ programCode }) {
    ...
    const fetchCoursesByProgram = async (programCode) => {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_URL}/api/course`);
            
            const filteredCourses = response.data.filter(course => course.codigo_programa === programCode);
    ...
    useEffect(() => {
        fetchCoursesByProgram(programCode);
    }, [programCode]);
